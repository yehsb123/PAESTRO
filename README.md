...
| **VS Code 명령 실행** (`executeCommand`) | ✅ 됨 |
| 오프라인 CLI(`pae`), 엔진 검색/오케스트레이션 | ✅ 됨 |
| **REST/MCP/CLI 실제 실행(end-to-end)** | ⏳ 미구현 — 현재는 검색·계획까지. 확장은 vscode 런타임만 실행하고 그 외는 "직접 실행 불가"로 안내 |
| `executeCommand` **인자 전달** | ⏳ 미구현 — 인자 스키마 보강 필요 |
